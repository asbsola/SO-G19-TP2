...
int initialize(char *diskname, char *size, char *mbr, char *boot, char *kernel) {
	...
	if (boot != NULL) {
		...
		if (kernel == NULL) { bootFileType = "system"; }
	}
	...
	if (ret == 0) {
		...
		while (writeSize < diskSize) {
			...
			if (chunkSize > diskSize - writeSize) { chunkSize = diskSize - writeSize; }
		...
		if (ret == 0) { printf("Formatting disk: %llu of %llu bytes (100%%)%9s\n", writeSize, diskSize, ""); }
	}
	...
	if (mbrFile != NULL) { fclose(mbrFile); }
	if (bootFile != NULL) { fclose(bootFile); }
	if (kernelFile != NULL) { fclose(kernelFile); }
	if (disk != NULL) {
		fclose(disk);
		disk = NULL;
	}
	...
	if (ret == 0) { printf("Disk initialization complete.\n"); }
