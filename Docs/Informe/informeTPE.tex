...
Al igual que en los semáforos, no existe una noción de pipe "abierto" dentro del espacio de memoria de un proceso, por lo que cualquier proceso puede acceder a un pipe conociendo su descriptor de archivo.
Al enviar un EOF a un pipe, este deja de ser escribible, permitiendo solo la lectura de los datos en el buffer previos al EOF. Además, empleamos dos pipes especiales para la pantalla y para el teclado, que, aunque no son pipes propiamente, tienen un comportamiento similar. Este diseño generaba un problema al enviar un EOF a la salida estándar, la misma quedaba in-utilizable. Para solucionarlo, añadimos un atributo especial \texttt{EOF\_CONSUMER} a los pipes, lo que permite un manejo adecuado de EOF.
...
\begin{itemize}
    \item test\_priority\_dist\: Este test crea múltiples procesos con diferentes prioridades y cuenta la cantidad de veces que ese proceso es elegido por el scheduler. Luego imprime cuantas veces fueron elegidos los procesos de cada una de las prioridades.
...
\begin{itemize}
    ...
    \item Al correr control + Shift + C, pueden quedar recursos abiertos por tanto leaks de memoria porque al no poder saber que recursos tiene un proceso, no podemos liberarlos.
\end{itemize}
