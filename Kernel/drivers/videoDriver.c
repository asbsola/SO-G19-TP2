...
void put_pixel(uint32_t hexColor, uint64_t x, uint64_t y) {
	if (y > get_video_buffer_height() || x > get_video_buffer_width()) return;
...
void draw_rect(uint32_t hexColor, uint32_t posX, uint32_t posY, uint32_t width, uint32_t height) {
	if (posX + width > get_video_buffer_width() || posY + height > get_video_buffer_height()) return;
...
void draw_char(char c, uint32_t hexColor, uint32_t posX, uint32_t posY) {
	for (uint32_t y = 0; y < CHAR_BIT_HEIGHT; y++) {
		for (uint32_t x = 0; x < CHAR_BIT_WIDTH; x++) {
			...
			if (bit) draw_square(hexColor, posX + x * screenTextInfo.fontSize, posY + y * screenTextInfo.fontSize, screenTextInfo.fontSize);
		}
	}
}
...
void update_screen_text_buffer() {
	...
	for (uint32_t y = screenTextInfo.indexY; y + 1 > lower_y_limit; y--)
		for (uint32_t x = 0; x < upper_x_limit; x++) draw_char(screenTextInfo.buffer[y][x].c, screenTextInfo.buffer[y][x].hexColor, x * fontWidth, (y - lower_y_limit) * fontHeight);
}
