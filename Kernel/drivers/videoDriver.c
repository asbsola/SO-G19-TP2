...
uint8_t video_buffer[MAX_VIDEO_BUFFER_WIDTH * MAX_VIDEO_BUFFER_HEIGHT *
                     MAX_VIDEO_BUFFER_BYTES_PER_PIXEL];

uint32_t get_video_buffer_width() {
  return (MAX_VIDEO_BUFFER_WIDTH > VBE_mode_info->width)
             ? VBE_mode_info->width
             : MAX_VIDEO_BUFFER_WIDTH;
}

uint32_t get_video_buffer_height() {
  return (MAX_VIDEO_BUFFER_HEIGHT > VBE_mode_info->height)
             ? VBE_mode_info->height
             : MAX_VIDEO_BUFFER_HEIGHT;
}

uint32_t get_video_buffer_bytes_per_pixel() {
  uint32_t bytesPerPixel = VBE_mode_info->bpp / 8;
  return (MAX_VIDEO_BUFFER_BYTES_PER_PIXEL > bytesPerPixel)
             ? bytesPerPixel
             : MAX_VIDEO_BUFFER_BYTES_PER_PIXEL;
}

void update_frame_buffer() {
  memcpy((void *)((uint64_t)VBE_mode_info->framebuffer), video_buffer,
         get_video_buffer_width() * get_video_buffer_height() *
             get_video_buffer_bytes_per_pixel());
}
...
void put_pixel(uint32_t hexColor, uint64_t x, uint64_t y) {
  ...
  uint64_t offset =
      (x * get_video_buffer_bytes_per_pixel()) + (y * VBE_mode_info->pitch);
  video_buffer[offset] = (hexColor) & 0xFF;
  video_buffer[offset + 1] = (hexColor >> 8) & 0xFF;
  video_buffer[offset + 2] = (hexColor >> 16) & 0xFF;
}

void draw_rect(uint32_t hexColor, uint32_t posX, uint32_t posY, uint32_t width,
               uint32_t height) {
  if (posX + width > get_video_buffer_width() ||
      posY + height > get_video_buffer_height())
    return;
...
void draw_square(uint32_t hexColor, uint32_t posX, uint32_t posY,
                 uint32_t size) {
  draw_rect(hexColor, posX, posY, size, size);
}

void clear_screen(uint32_t clearColor) {
  draw_rect(clearColor, 0, 0, get_video_buffer_width(),
            get_video_buffer_height());
}
...
uint32_t get_chars_per_buff_line() {
  ...
  return (charsPerLine <= SCREEN_TEXT_BUFFER_WIDTH) ? charsPerLine
                                                    : SCREEN_TEXT_BUFFER_WIDTH;
}
...
void draw_char(char c, uint32_t hexColor, uint32_t posX, uint32_t posY) {
  for (uint32_t y = 0; y < CHAR_BIT_HEIGHT; y++) {
    for (uint32_t x = 0; x < CHAR_BIT_WIDTH; x++) {
      uint8_t bit =
          font_bitmap[c * CHAR_BIT_HEIGHT + y] & (1 << (CHAR_BIT_WIDTH - x));
      if (bit)
        draw_square(hexColor, posX + x * screenTextInfo.fontSize,
                    posY + y * screenTextInfo.fontSize,
                    screenTextInfo.fontSize);
    }
  }
}

void draw_string(const char *str, uint32_t len, uint32_t hexColor,
...
void write_to_video_text_buffer(const char *data, uint32_t data_len,
                                uint32_t hexColor) {
  for (uint32_t i = 0; i < data_len; i++) {
    switch (data[i]) {
    case '\e':
      hexColor = hex_color_string_to_number(&data[i + 1]);
      i += (data_len - HEX_COLOR_LEN != 0) ? HEX_COLOR_LEN
                                           : (data_len - HEX_COLOR_LEN);
      break;
    case '\n':
      screenTextInfo.indexY += 1;

      if (screenTextInfo.indexY == SCREEN_TEXT_BUFFER_HEIGHT)
    ...
    default:
      screenTextInfo.buffer[screenTextInfo.indexY][screenTextInfo.indexX].c =
          data[i];
      screenTextInfo.buffer[screenTextInfo.indexY][screenTextInfo.indexX]
          .hexColor = hexColor;

      screenTextInfo.indexX += 1;
      screenTextInfo.indexY +=
...
void update_screen_text_buffer() {
  ...
  uint32_t lower_y_limit = (linesPerScreen <= screenTextInfo.indexY)
                               ? screenTextInfo.indexY - linesPerScreen
                               : 0;
  ...
  for (uint32_t y = screenTextInfo.indexY; y + 1 > lower_y_limit; y--)
    for (uint32_t x = 0; x < upper_x_limit; x++)
      draw_char(screenTextInfo.buffer[y][x].c,
                screenTextInfo.buffer[y][x].hexColor, x * fontWidth,
                (y - lower_y_limit) * fontHeight);
}
