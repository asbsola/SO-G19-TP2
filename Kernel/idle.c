...
void remove_orphans(processManagerADT process_manager) {
	...
	for (int i = get_max_pid(process_manager); i >= 0; i--) {
		if (processes[i] != NULL && processes[i]->parent_pid == IDLE_PROCESS_PID &&
...
uint64_t idle(char **argv, int argc) {
	...
	pid_t screen_pid = create_process(the_process_manager, 0, screen_service,
			screen_service_args, SCREEN_OUTPUT_FD, -1);
	nicent(the_process_manager, screen_pid, HIGH);

	char *shell_args[] = {"shell", NULL};
	create_process(the_process_manager, 0,
			(uint64_t(*)(char **, int))(SHELL_CODE_ADDRESS), shell_args,
