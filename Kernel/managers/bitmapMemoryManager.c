...
#define USED 1
#define START 3
#define GET_BLOCK_STATE(val, b)                                                \
  (((val) & (3 << ((b) * BITS_PER_BLOCK))) >> ((b) * BITS_PER_BLOCK))
...
memoryManagerADT init_memory_manager(void *memory, uint64_t memory_size) {
  ...
  uint64_t bitmap_max_length = (memory_size - sizeof(struct memoryManagerCDT)) /
                               BLOCK_SIZE / blocks_per_entry;

  uint64_t non_usable_memory_size =
      sizeof(struct memoryManagerCDT) +
      bitmap_max_length * sizeof(mem_manager->bitmap[0]);
  ...
  mem_manager->bitmap_length =
      mem_manager->max_blocks / blocks_per_entry +
      (mem_manager->max_blocks % blocks_per_entry != 0);
  mem_manager->memory_start =
      mem_manager->bitmap_start +
      mem_manager->bitmap_length * sizeof(mem_manager->bitmap[0]);
...
int find_free_blocks(memoryManagerADT mem_manager, uint64_t blocks_needed,
...
void reserve_blocks(memoryManagerADT mem_manager, uint64_t start_block,
                    uint64_t blocks) {
  int blocks_per_entry = sizeof(mem_manager->bitmap[0]) * BLOCKS_PER_BYTE;
...
void free_blocks(memoryManagerADT mem_manager, uint64_t start_block) {
  ...
  uint64_t index = start_block / blocks_per_entry;
  int block_offset = start_block % blocks_per_entry;
  for (uint64_t b = 0; b == 0 || GET_BLOCK_STATE(mem_manager->bitmap[index],
                                                 block_offset) == USED;
       b++) {
