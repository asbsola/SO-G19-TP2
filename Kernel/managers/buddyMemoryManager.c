...
memoryManagerADT init_memory_manager(void *memory, uint64_t memory_size) {
	...
	if (max_block_order < MIN_BLOCK_SIZE_ORDER) return NULL;
	...
	if (usable_memory < min_block_size) return NULL;
	...
	for (uint64_t i = 0; i < tree_max_node_count; i++) root[i].status = FREE;
...
void *allocate_mem_recursive(memoryNode *tree, uint64_t size, uint64_t chunk_size, void *chunk_start, uint64_t node_index, uint64_t height) {
	if (chunk_size < size || tree[node_index].status == USED) return NULL;

	if (chunk_size == size) {
		if (tree[node_index].status == SPLIT) return NULL;

		tree[node_index].status = USED;
		return chunk_start;
	}

	if (tree[node_index].status != SPLIT) { tree[node_index].status = SPLIT; }
...
int is_valid_dir(memoryManagerADT mem_manager, void *address) {
	if (address < mem_manager->memory_start || address >= mem_manager->memory_start + mem_manager->memory_size) { return 0; }
...
int64_t free_mem_recursive(memoryNode *tree, void *address, uint64_t chunk_size, void *chunk_start, uint64_t node_index, uint64_t height) {
	...
	if (address == chunk_start) {
		...
		if (address < chunk_start) return -1;

		out = free_mem_recursive(tree, address, half_size, chunk_start + half_size, right_index, height - 1);

		if (out < 0) { out = free_mem_recursive(tree, address, half_size, chunk_start, left_index, height - 1); }
	}

	if (tree[left_index].status == FREE && tree[right_index].status == FREE) tree[node_index].status = FREE;

	return out;
}

void mem_free(memoryManagerADT mem_manager, void *address) {
	if (!is_valid_dir(mem_manager, address)) return;
