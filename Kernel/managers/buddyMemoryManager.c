...
memoryManagerADT init_memory_manager(void *memory, uint64_t memory_size) {
  ...
  mem_manager->memory_start =
      (void *)(((uint64_t)(mem_manager->tree_memory_start + tree_max_size) /
                    ALIGNMENT +
                1) *
               ALIGNMENT);
...
uint64_t get_total_memory_size(memoryManagerADT mem_manager) {
  return (uint64_t)((uint64_t)mem_manager->memory_start -
                    (uint64_t)mem_manager) +
         mem_manager->memory_size;
}

void *allocate_mem_recursive(memoryNode *tree, uint64_t size,
                             uint64_t chunk_size, void *chunk_start,
                             uint64_t node_index, uint64_t height) {
  ...
  void *addr = allocate_mem_recursive(tree, size, half_size, chunk_start,
                                      left_index, height - 1);
  if (addr == NULL) {
    uint64_t right_index = node_index + _2pown(height - 1);
    addr =
        allocate_mem_recursive(tree, size, half_size, chunk_start + half_size,
                               right_index, height - 1);
  }
...
void *mem_alloc(memoryManagerADT mem_manager, uint64_t size) {
  ...
  void *out = allocate_mem_recursive(
      mem_manager->root, best_fit_size, mem_manager->memory_size,
      mem_manager->memory_start, 0, mem_manager->tree_max_height);
  mem_manager->free_memory_size -= (out != NULL) ? best_fit_size : 0;
  return out;
}

int is_valid_dir(memoryManagerADT mem_manager, void *address) {
  if (address < mem_manager->memory_start ||
      address >= mem_manager->memory_start + mem_manager->memory_size) {
    return 0;
  }
...
int64_t free_mem_recursive(memoryNode *tree, void *address, uint64_t chunk_size,
                           void *chunk_start, uint64_t node_index,
                           uint64_t height) {
  uint64_t half_size = chunk_size / 2;
  uint64_t left_index = node_index + 1;
  uint64_t right_index = node_index + _2pown(height - 1);
  ...
  if (address == chunk_start) {
    ...
    out = free_mem_recursive(tree, address, half_size, chunk_start, left_index,
                             height - 1);
  } else {
    if (address < chunk_start)
      return -1;

    out = free_mem_recursive(tree, address, half_size, chunk_start + half_size,
                             right_index, height - 1);

    if (out < 0) {
      out = free_mem_recursive(tree, address, half_size, chunk_start,
                               left_index, height - 1);
    }
  }
...
void mem_free(memoryManagerADT mem_manager, void *address) {
  ...
  uint64_t freed_size = free_mem_recursive(
      mem_manager->root, address, mem_manager->memory_size,
      mem_manager->memory_start, 0, mem_manager->tree_max_height);
