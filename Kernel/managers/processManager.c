...
processManagerADT init_process_manager(memoryManagerADT memory_manager,
		schedulerADT scheduler) {
	processManagerADT process_manager =
		mem_alloc(memory_manager, sizeof(struct processManagerCDT));
	...
	char *argv[] = {"idle", NULL};
	create_process(process_manager, -1, idle, argv, KEYBOARD_INPUT_FD,
			SCREEN_OUTPUT_FD);
...
int is_child(processManagerADT process_manager, pid_t my_pid, pid_t child_pid) {
	if (invalid_pid(process_manager, my_pid) ||
			invalid_pid(process_manager, child_pid))
		return 0;
...
pid_t get_lowest_unused_pid(processManagerADT process_manager) {
	pid_t pid;
	for (pid = 0; pid <= process_manager->max_pid &&
			process_manager->processes[pid] != NULL;
			pid++)
		;
...
pid_t create_process(processManagerADT process_manager, pid_t parent_pid,
		uint64_t (*process_start)(char **, int), char **argv,
		fd_t stdin, fd_t stdout) {
	if (process_manager->num_processes >= MAX_PROCESSES)
		return -1;

	processControlBlockADT process_pcb = mem_alloc(
			process_manager->memory_manager, sizeof(processControlBlockCDT));
	...
	struct startFrame *start_frame =
		(startFrame *)(process_pcb->stack + PROCESS_STACK_SIZE -
				sizeof(startFrame));
	...
	registers64_t *call_frame =
		(registers64_t *)(process_pcb->stack + PROCESS_STACK_SIZE -
				sizeof(startFrame) - sizeof(registers64_t));
	call_frame->rip = (uint64_t)start_process_wrapper;
	call_frame->rsp =
		(uint64_t)(process_pcb->stack + PROCESS_STACK_SIZE - sizeof(startFrame));
	call_frame->ss = 0x0;
	call_frame->cs = 0x8;
	call_frame->rflags = 0x202;

	process_pcb->rsp = (uint64_t)(process_pcb->stack + PROCESS_STACK_SIZE -
			sizeof(startFrame) - sizeof(registers64_t));
...
int check_finnished_child(processManagerADT process_manager, pid_t my_pid,
		pid_t child_pid, int64_t *ret,
		processStatus *status) {
	if (!is_child(process_manager, my_pid, child_pid) ||
			!has_finnished(process_manager, child_pid))
		return 0;
...
int check_finnished_children(processManagerADT process_manager, pid_t my_pid,
		int64_t *ret, processStatus *status) {
	for (pid_t pid = 0; pid <= process_manager->max_pid; pid++) {
...
int exit_process(processManagerADT process_manager, pid_t pid, int64_t status) {
	...
	if (get_current_pid(process_manager->scheduler) == pid)
		...
		deschedule_process(process_manager->scheduler,
...
void kill_recursive(processManagerADT process_manager, pid_t my_pid,
		uint64_t recursive) {
	for (pid_t pid = 0; pid <= process_manager->max_pid; pid++)
		if (is_child(process_manager, my_pid, pid))
...
int kill_process(processManagerADT process_manager, pid_t pid,
	...
	if (!recursive)
		...
		kill_recursive(process_manager, pid, recursive);
...
uint64_t kill_signal(processManagerADT process_manager, int recursive) {
	for (pid_t pid = 0; pid <= process_manager->max_pid; pid++)
		if (pid != IDLE_PROCESS_PID &&
				...
				get_grandparent_pid(process_manager, pid) == IDLE_PROCESS_PID)
...
uint64_t wait_process(processManagerADT process_manager, pid_t child_pid,
		int64_t *ret) {
	pid_t my_pid = get_current_pid(process_manager->scheduler);
...
processControlBlockADT get_process(processManagerADT process_manager,
		pid_t pid) {
	if (invalid_pid(process_manager, pid))
		return NULL;
...
uint64_t get_ps_data(processManagerADT process_manager,
	...
	process_info_t *processes = (process_info_t *)mem_alloc(
			mem_manager,
			sizeof(process_info_t) * (process_manager->num_processes + 1));
	int index = 0;
	for (int i = 0; i <= process_manager->max_pid; i++) {
		if (process_manager->processes[i] != NULL) {
			...
			processes[index].stack_pointer =
				(void *)process_manager->processes[i]->rsp;
			processes[index].base_pointer = process_manager->processes[i]->stack;
			processes[index].status = process_manager->processes[i]->status;
			processes[index].parent_is_waiting =
				process_manager->processes[i]->parent_is_waiting;
...
char **copy_argv(processManagerADT processes_manager, char **argv) {
	...
	char **argv_copy = (char **)mem_alloc(processes_manager->memory_manager,
			(size_of_argv + 1) * sizeof(char *));
	...
	for (uint64_t i = 0; i < size_of_argv; i++) {
		...
		char *arg = (char *)mem_alloc(processes_manager->memory_manager,
				(len + 1) * sizeof(char));
		if (arg == NULL) {
...
uint64_t nicent(processManagerADT process_manager, pid_t pid,
	...
	change_process_priority(process_manager->scheduler,
			process_manager->processes[pid],
			process_manager->processes[pid]->priority, priority);
	return 0;
}
