...
processManagerADT init_process_manager(memoryManagerADT memory_manager, schedulerADT scheduler) {
	...
	if (process_manager == NULL) return NULL;

	for (pid_t pid = 0; pid < MAX_PROCESSES; pid++) process_manager->processes[pid] = NULL;
...
int is_child(processManagerADT process_manager, pid_t my_pid, pid_t child_pid) {
	if (invalid_pid(process_manager, my_pid) || invalid_pid(process_manager, child_pid)) return 0;
...
int has_children(processManagerADT process_manager, pid_t my_pid) {
	for (pid_t pid = 0; pid <= process_manager->max_pid; pid++) {
		if (is_child(process_manager, my_pid, pid)) return 1;
	}
	return 0;
}
...
pid_t create_process(processManagerADT process_manager, pid_t parent_pid, uint64_t (*process_start)(char **, int), char **argv, fd_t stdin, fd_t stdout) {
	if (process_manager->num_processes >= MAX_PROCESSES) return -1;
	...
	if (process_pcb == NULL || stack == NULL) return -1;
...
void check_waiting_parent(processManagerADT process_manager, pid_t pid) {
	...
	if (process_manager->processes[pid]->parent_is_waiting == WAITING) unblock_process(process_manager, parent_pid);
}
...
void stop_waiting_parent(processManagerADT process_manager, pid_t parent_pid) {
	for (pid_t pid = 0; pid <= process_manager->max_pid; pid++)
		if (is_child(process_manager, parent_pid, pid)) process_manager->processes[pid]->parent_is_waiting = NOT_WAITING;
}
...
void update_childs_ppid(processManagerADT process_manager, pid_t my_pid) {
	for (pid_t pid = 0; pid <= process_manager->max_pid; pid++)
		if (is_child(process_manager, my_pid, pid)) process_manager->processes[pid]->parent_pid = IDLE_PROCESS_PID;
}
...
int unblock_process(processManagerADT process_manager, pid_t pid) {
	if (invalid_pid(process_manager, pid) || process_manager->processes[pid]->status != BLOCKED) return -1;
...
void kill_recursive(processManagerADT process_manager, pid_t my_pid, uint64_t recursive) {
	for (pid_t pid = 0; pid <= process_manager->max_pid; pid++)
		if (is_child(process_manager, my_pid, pid)) kill_process(process_manager, pid, recursive);
}

int kill_process(processManagerADT process_manager, pid_t pid, uint64_t recursive) {
	if (pid == IDLE_PROCESS_PID || invalid_pid(process_manager, pid)) return -1;
...
uint64_t wait(processManagerADT process_manager, int64_t *ret) {
	...
	if (!has_children(process_manager, my_pid)) return -1;

	for (pid_t pid = 0; pid <= process_manager->max_pid; pid++)
		if (is_child(process_manager, my_pid, pid)) process_manager->processes[pid]->parent_is_waiting = WAITING;

	processStatus status;
	if (check_finnished_children(process_manager, my_pid, ret, &status)) return status;
...
uint64_t wait_process(processManagerADT process_manager, pid_t child_pid, int64_t *ret) {
	...
	if (!is_child(process_manager, my_pid, child_pid)) return -1;

	process_manager->processes[child_pid]->parent_is_waiting = WAITING;

	processStatus status;
	if (check_finnished_child(process_manager, my_pid, child_pid, ret, &status)) return status;
	block_process(process_manager, my_pid);
	if (check_finnished_child(process_manager, my_pid, child_pid, ret, &status)) return status;
...
char **copy_argv(processManagerADT processes_manager, char **argv) {
	...
	if (argv_copy == NULL) return fallback;
	...
	for (uint64_t i = 0; i < size_of_argv; i++) {
		...
		if (arg == NULL) {
			for (uint64_t j = 0; j < i; j++) mem_free(processes_manager->memory_manager, argv_copy[j]);
...
void free_argv(processManagerADT process_manager, char **argv) {
	for (uint64_t i = 0; argv[i] != NULL; i++) mem_free(process_manager->memory_manager, argv[i]);
