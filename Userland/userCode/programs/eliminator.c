...
void scan_player_name(char *text, int len, uint8_t player_number,
  ...
  for (i = 0; i < MAX_LENGTH_PLAYER_NAME && key != '\n'; i++) {
    ...
    sys_put_text(dest, len + i + 1, HEX_RED, 100,
                 250 + 50 * (player_number - 1));
  }
  player->name[i] = '\0';
  player->name_len = i;
}
...
void changeDirection(char key, int *p1Dir, int *p2Dir) {
  ...
  case W_KEY_CODE_PRESSED:
    if (player1Data.direction != DOWN_DIRECTION)
      *p1Dir = UP_DIRECTION;
    break;
  case D_KEY_CODE_PRESSED:
    if (player1Data.direction != LEFT_DIRECTION)
      *p1Dir = RIGHT_DIRECTION;
    break;
  case S_KEY_CODE_PRESSED:
    if (player1Data.direction != UP_DIRECTION)
      *p1Dir = DOWN_DIRECTION;
    break;
  case A_KEY_CODE_PRESSED:
    if (player1Data.direction != RIGHT_DIRECTION)
      *p1Dir = LEFT_DIRECTION;
    break;
  case UP_ARROW_CODE_PRESSED:
    if (player2Data.direction != DOWN_DIRECTION)
      *p2Dir = UP_DIRECTION;
    break;
  case RIGHT_ARROW_CODE_PRESSED:
    if (player2Data.direction != LEFT_DIRECTION)
      *p2Dir = RIGHT_DIRECTION;
    break;
  case DOWN_ARROW_CODE_PRESSED:
    if (player2Data.direction != UP_DIRECTION)
      *p2Dir = DOWN_DIRECTION;
    break;
  case LEFT_ARROW_CODE_PRESSED:
    if (player2Data.direction != RIGHT_DIRECTION)
      *p2Dir = LEFT_DIRECTION;
    break;
  default:
    break;
  }
...
void play() {
  ...
  while (keep_playing == 1) {
    ...
    for (uint32_t x = 1; x < normalized_screen_width - 1; x++) {
      ...
      sys_draw_square(HEX_RED, x * SQUARE_SIZE,
                      (normalized_screen_height - 2) * SQUARE_SIZE,
                      SQUARE_SIZE);
    }

    for (uint32_t y = 1; y < normalized_screen_height - 1; y++) {
      sys_draw_square(HEX_RED, SQUARE_SIZE, y * SQUARE_SIZE, SQUARE_SIZE);
      sys_draw_square(HEX_RED, (normalized_screen_width - 2) * SQUARE_SIZE,
    ...
    while (player1Data.alive == 1 && (players != 2 || player2Data.alive == 1)) {
      ...
      player1Data.x +=
          (player1Data.direction == 1) - (player1Data.direction == 3);
      player1Data.y +=
          (player1Data.direction == 2) - (player1Data.direction == 0);

      player2Data.x +=
          (player2Data.direction == 1) - (player2Data.direction == 3);
      player2Data.y +=
          (player2Data.direction == 2) - (player2Data.direction == 0);

      if (player1Data.x > bufferWidth - 1 || player1Data.y > bufferHeight - 1 ||
          buffer[player1Data.y][player1Data.x] == 1) {
        player1Data.alive = 0;
      }

      if (player2Data.x > bufferWidth - 1 || player2Data.y > bufferHeight - 1 ||
      ...
      if (player1Data.alive) {
        buffer[player1Data.y][player1Data.x] = 1;
        sys_draw_square(player1Data.color, (player1Data.x + 2) * SQUARE_SIZE,
                        (player1Data.y + 2) * SQUARE_SIZE, SQUARE_SIZE);
      }

      if (players == 2 && player2Data.alive) {
        buffer[player2Data.y][player2Data.x] = 1;
        sys_draw_square(player2Data.color, (player2Data.x + 2) * SQUARE_SIZE,
                        (player2Data.y + 2) * SQUARE_SIZE, SQUARE_SIZE);
      }
...
int endGame() {
  ...
  if (players == 2) {
    if (player1Data.alive == 0 && player2Data.alive == 1) {
      ...
      int n = 1;
      int i;
      for (i = 1; n * 10 <= player2Data.score && i < MAX_DIGITS_SCORE; i++)
        n *= 10;
      ...
      int n = 1;
      int i;
      for (i = 1; n * 10 <= player1Data.score && i < MAX_DIGITS_SCORE; i++)
        n *= 10;
  ...
  sys_put_text(dest, player1Data.name_len + 2 + player1Data.score_digits,
               player1Data.color, 100, 100);

  if (players == 2) {
    strcat(dest, player2Data.name, ": ");
    strcat(dest, dest,
           itoa(player2Data.score, number, player2Data.score_digits));
    sys_put_text(dest, player2Data.name_len + 2 + player2Data.score_digits,
                 player2Data.color, 100, 150);
  }
