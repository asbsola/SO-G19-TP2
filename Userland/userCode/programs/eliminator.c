...
void change_settings_and_play() {
	...
	char key = 0;
	while (key < '1' || key > '5') { key = sys_get_character_pressed(WAITING); }
	speed = key - '0';
	...
	key = 0;
	while (key != '1' && key != '2') { key = sys_get_character_pressed(WAITING); }
	players = key - '0';
	print_setting("PLAYERS (1-2): ", 16, players, HEX_RED, 100, 150);

	scan_player_name("PLAYER 1: ", 10, 1, &player1Data);

	if (players == 2) { scan_player_name("PLAYER 2: ", 10, 2, &player2Data); }
...
void play() {
	...
	while (keep_playing == 1) {
		...
		for (int i = 0; i < bufferHeight; i++)
			for (int j = 0; j < bufferWidth; j++) buffer[i][j] = 0;
		...
		while (player1Data.alive == 1 && (players != 2 || player2Data.alive == 1)) {
			...
			while ((key = sys_get_key_pressed(NOT_WAITING)) != 0) { changeDirection(key, &player1Dir, &player2Dir); }
			...
			if (player1Data.x > bufferWidth - 1 || player1Data.y > bufferHeight - 1 || buffer[player1Data.y][player1Data.x] == 1) { player1Data.alive = 0; }

			if (player2Data.x > bufferWidth - 1 || player2Data.y > bufferHeight - 1 || buffer[player2Data.y][player2Data.x] == 1) { player2Data.alive = 0; }
...
uint64_t eliminator(char **argv, int argc) {
	...
	for (int i = 0; i < bufferHeight; i++) sys_free(buffer[i]);
	sys_free(buffer);
