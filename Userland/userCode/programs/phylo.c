...
uint64_t view(char **argv, int argc) {
  ...
  int len = 0;
  int read = 0;
  while ((read = sys_read(view_pipe, &buffer[len++], 1)) != EOF &&
         len <= MAX_NUMBER_OF_PHILOSOPHERS) {
    if (buffer[len - 1] == '\0') {
      puts(buffer);
      puts("\n");
      len = 0;
...
int add_philosopher() {
  ...
  if ((philosophers[number_of_philosophers - 1].pid = sys_create_process(
           thinking_man, argv, KEYBOARD_INPUT_FD, view_pipe)) == -1) {
...
uint64_t phylo(char **argv, int argc) {
  ...
  if (in_background) {
    puts_with_color(
        "phylo: ERROR don't run me in background (no exit condition)\n",
        0xFF0000);
    return -1;
  }

  puts_with_color("phylo instructions: 'a' to add - 'r' to remove - 'q' to "
                  "quit\n\n's' to start:\n",
                  0xc2daff);
  while (getchar() != 's')
    ;
