...
void putchar(char c) { sys_write(sys_get_stdout(), &c, 1); }

void puts(const char *s) { sys_write(sys_get_stdout(), s, strlen(s) + 1); }
...
void printf(const char *fmt, ...) {
  ...
  for (uint32_t i = 0; k < PRINTF_PRINT_BUFF_MAX_SIZE && fmt[i] != '\0'; i++) {
    if (fmt[i] == '%' && fmt[i + 1] == 's') {
      ...
      for (uint32_t j = 0; k < PRINTF_PRINT_BUFF_MAX_SIZE && arg_s[j] != '\0';
           j++)
      ...
      for (uint32_t j = 0; k < PRINTF_PRINT_BUFF_MAX_SIZE && temp[j] != '\0';
           j++)
      ...
      for (uint64_t j = 0; k < PRINTF_PRINT_BUFF_MAX_SIZE && temp[j] != '\0';
           j++)
        printf_buff[k++] = temp[j];
...
char getchar() {
  char c = 0;
  if (sys_read(sys_get_stdin(), &c, 1) == EOF)
    return 0;
  return c;
}
...
int getline(char *buffer, int max_len) {
  int len = 0;
  while (sys_read(sys_get_stdin(), buffer + len, 1) != EOF && len < max_len &&
         buffer[len] != '\n')
    len++;
  buffer[len++] = '\n';
  return len;
