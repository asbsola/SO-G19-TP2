...
uint64_t test_idle_cleanup(char **argv, int argc) {
	...
	if (argc < 2 || (children_count = atoi(argv[1])) <= 0) {
		puts_with_color(
				"test_idle_cleanup: ERROR must provide max_processes (tops at 250)\n",
				0xFF0000);
		return -1;
	}
	...
	children_count =
		(children_count > TOTAL_PROCESSES) ? TOTAL_PROCESSES : children_count;
	...
	for (uint64_t i = 0; i < children_count; i++) {
		sys_create_process(bussy_wait_proc, argvAux, KEYBOARD_INPUT_FD,
				SCREEN_OUTPUT_FD);
	}
