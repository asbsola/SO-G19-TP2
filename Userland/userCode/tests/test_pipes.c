...
uint64_t mega_test_pipes(char **argv, int argc) {
  for (int i = 250; i > 0; i--) {
    for (int j = 125; j > 0; j--) {
      ...
      pid_t pid = sys_create_process(test_pipes, auxArgv, sys_get_stdin(),
                                     sys_get_stdout());
      int64_t ret;
...
uint64_t test_pipes(char **argv, int argc) {
  ...
  if (argc < 3) {
    puts_with_color("test_pipes: ERROR must provide msg_len and "
                    "max_pair_processes (tops at 125)\n",
                    0xFF0000);
    return -1;
  }
  ...
  if (max_pair_processes <= 0) {
    puts_with_color(
        "test_pipes: ERROR max_pair_processes must be greater than 0\n",
        0xFF0000);
    return -1;
  }
  if (max_pair_processes > TOTAL_PAIR_PROCESSES) {
    puts_with_color(
        "test_pipes: ERROR max_pair_processes must be less than 250\n",
        0xFF0000);
    return -1;
  }
