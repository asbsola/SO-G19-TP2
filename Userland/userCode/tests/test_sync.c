...
uint64_t test_sync(char **argv, int argc) {
  ...
  if (process_count < 0 || process_count > (MAX_PROCESSES / 2)) {
    puts_with_color("test_sync: ERROR max_pair_processes must be greater than "
  ...
  for (i = 0; i < process_count; i++) {
    pids[i] = sys_create_process(process_inc, argvDec, KEYBOARD_INPUT_FD,
                                 SCREEN_OUTPUT_FD);
    pids[i + process_count] = sys_create_process(
        process_inc, argvInc, KEYBOARD_INPUT_FD, SCREEN_OUTPUT_FD);
